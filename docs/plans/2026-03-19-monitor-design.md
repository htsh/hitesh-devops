...
- [Monitor Dashboard Implementation Plan](./2026-03-19-monitor-dashboard-implementation-plan.md)

## 1. Goal

Build a self-hosted infrastructure monitor that runs on `vps3`, stores its data in Mongo, monitors the current infrastructure described in [Infrastructure Inventory](../infra/infra.md), provides an internal admin dashboard, and sends `ntfy` notifications when something goes down.
...
Initial monitored targets should be derived from [Infrastructure Inventory](../infra/infra.md).
...
- heartbeat checks for cron jobs and scheduled tasks
- PostgreSQL checks later without redesigning the target model
...
- count of underlying checks
- affected node or footprint summary
- whether any outage is currently open for that service
...
- service summary
- current checks grouped under the service
- current service health
- recent outages affecting that service
...
- The initial infrastructure targets from [Infrastructure Inventory](../infra/infra.md) can be represented in the system.
